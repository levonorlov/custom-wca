...
const config = {
  "addon:m110_mag_empty": { // Предмет который будет чиниться.
  ...
  if (item) {
    ...
    system.run(() => {
      ...
      if (stickIndex && currentDurability === 0) {
        player.runCommand(`clear @s ${item.mag} 0 1`);
        player.runCommand(`replaceitem entity @s slot.weapon.mainhand 0 ${itemStack.typeId} 1 19`);
        player.runCommand(`playsound addon.patron @a ^^^0.10 1 1`);
      }
      if (stickIndex && currentDurability === 1) {
        player.runCommand(`clear @s ${item.mag} 0 1`);
        player.runCommand(`replaceitem entity @s slot.weapon.mainhand 0 addon:m110_mag 1`);
        player.runCommand(`playsound addon.patron @a ^^^0.10 1 1`);
      }
    });
  }
