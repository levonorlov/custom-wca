...
const config = {
  "addon:ak74_mag_empty": { // Предмет который будет чиниться.
    "mag": "addon:ammo5.45x39", // Магазин который будет чинить.
  ...
  if (item) {
    ...
    system.run(() => {
      ...
      if (stickIndex && currentDurability === 0) {
        player.runCommand(`clear @s ${item.mag} 0 1`);
        player.runCommand(`replaceitem entity @s slot.weapon.mainhand 0 ${itemStack.typeId} 1 29`);
        player.runCommand(`playsound addon.patron @a ^^^0.10 1 1`);
      }
      if (stickIndex && currentDurability === 1) {
        player.runCommand(`clear @s ${item.mag} 0 1`);
        player.runCommand(`replaceitem entity @s slot.weapon.mainhand 0 addon:ak74_mag 1`);
        player.runCommand(`playsound addon.patron @a ^^^0.10 1 1`);
      }
    });
  }
