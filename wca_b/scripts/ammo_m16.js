...
const config = {
  "addon:m16_mag_empty": { // Предмет который будет чиниться.
    "mag": "addon:ammo5.56x45", // Магазин который будет чинить.
  ...
  if (item) {
    ...
    system.run(() => {
      ...
      if (stickIndex && currentDurability === 1) {
        player.runCommand(`clear @s ${item.mag} 0 1`);
        player.runCommand(`replaceitem entity @s slot.weapon.mainhand 0 addon:m16_mag 1`);
        player.runCommand(`playsound addon.patron @a ^^^0.10 1 1`);
      }
    });
  }
